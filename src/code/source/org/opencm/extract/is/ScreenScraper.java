...
public class ScreenScraper {
	...
	public WxConfigInfo getWxConfigInfo() {
		WxConfigInfo wxConfigInfo = new WxConfigInfo();
		try {
			// Set timeouts
			this.driver.manage().timeouts().implicitlyWait(30, java.util.concurrent.TimeUnit.SECONDS);
			// Get info page
			this.driver.get(this.baseURL + "/invoke/wx.config.pub/getAllValues");

			LogUtils.log(opencmConfig.getDebug_level(),Configuration.OPENCM_LOG_TRACE,"getJCE :: " + this.driver.getCurrentUrl());
			
			String ssValue = this.driver.findElement(By.xpath("//b[text()='keyValuePairs']/following::td")).getText();
			if (ssValue != null) {
				wxConfigInfo.setKeyValues(ExtractKeyValue.getKeyValuePairs(ssValue));
			}
